...
class Validacion
{
    ...
    protected function _apellidos($campo, $valor){

        if($valor!=""){

            if (preg_match("/^[A-Za-zñÑ]*[\s]*[A-Za-zñÑ]*$/", $valor))
            return true;
            else {
                $this->mensaje[$campo][] = "El campo $campo deben de ser solo letras";
                return false;
    ...
    protected function _telefono($campo, $valor){
        if($valor !=""){
            if(!preg_match("/^[0-9]{9}$/", $valor)){
                $this->mensaje[$campo][] = "Teléfono mal introducido";
                return false;
            }else{
                return true;
            }
    ...
    protected function _numCuenta($campo, $valor){

        if($valor !=""){
            if(!preg_match("/^(ES)(\d\d)\s(\d\d\d\d)\s(\d\d\d\d)\s(\d\d\d\d)\s(\d\d\d\d)$/", $valor)){
                $this->mensaje[$campo][] = "Número de cuenta incorrecto";
                return false;
            }else{
                return true;
            }
